    ...
    function(stdout)
      ...
      for i = 1, battery_number do
        ...
        awful.spawn.easy_async_with_shell(
          ...
          function(stdout)
            battery_status[i].status = stdout
            awful.spawn.easy_async_with_shell(
            "cat /sys/class/power_supply/BAT" .. tostring(i - 1) .. "/capacity",
            function(stdout)
              battery_status[i].percentage = stdout
...
local function create_battery_widget()
  ...
  local bat_percentage_value = 50

  local battery_percentage_text = wibox.widget {
    text = tostring(bat_percentage_value) .. "%",
    font = "SauceCodePro 8",
    id = "percentage_text",
    align = "center",
    valign = "center",
  ...
  local battery_tooltip = awful.tooltip{
    ...
    timer_function = function()
      local str = ""
      for i = 1, battery_number do 
        str = str .. "Battery " .. tostring(i) .. ": " .. battery_status[i].percentage .. "% " .. battery_status[i].status .. "\n"
      end
  ...
  local function update_progressbar()
    ...
    battery_percentage_text.text = tostring(val) .. "%"
